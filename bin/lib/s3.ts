...
export async function addPublicKey(region: string, userName: string, publicKey: string): Promise<Array<{ [key: string]: string }>> {
  AWS.config.update({ region });
  const iam = new AWS.IAM();
...
export async function deletePublicKey(region: string, userName: string, publicKeyId: string): Promise<Array<{ [key: string]: string }>> {
  AWS.config.update({ region });
  const iam = new AWS.IAM();
